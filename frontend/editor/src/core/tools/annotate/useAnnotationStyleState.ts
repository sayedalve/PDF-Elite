...
export const useAnnotationStyleState = (
  ...
  const [noteBackgroundColor, setNoteBackgroundColorState] = useState(() => {
    return localStorage.getItem("pdf-elite-note-color") || "#ffd54f";
  });
  const setNoteBackgroundColor = useCallback((color: string) => {
    setNoteBackgroundColorState(color);
