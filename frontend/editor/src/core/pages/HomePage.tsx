...
import { PrimaryModeRail } from "@app/components/shared/PrimaryModeRail";
...
export default function HomePage() {
  ...
  const hideToolPanel =
    (activeFiles.length === 0 && navigationState.workbench !== "myFiles") ||
    activeFiles.length > 0 ||
    navigationState.workbench === "viewer" ||
    (customWorkbenchViews.find(
      (v) => v.workbenchId === navigationState.workbench,
    )?.hideToolPanel ??
      false);

  const brandAltText = t("home.mobile.brandAlt", "PDF Elite logo");
  ...
  const appName = config?.appNameNavbar || "PDF Elite";
  useDocumentMeta({
