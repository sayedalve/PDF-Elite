...
export interface AutomationFormActions {
  ...
  updateOperationParams: (index: number, params: Record<string, unknown>) => void;
...
export function useAutomationForm(
  ...
  const addOperation = useCallback((tool: AutomationTool) => {
    setOperations((prev) => [
      ...prev,
      { operation: tool.operation, parameters: tool.parameters ?? {} },
    ]);
  }, []);
  ...
  const updateOperationParams = useCallback(
    (index: number, params: Record<string, unknown>) => {
      setOperations((prev) =>
        prev.map((op, i) => (i === index ? { ...op, parameters: params } : op)),
      );
    },
    [],
  );
