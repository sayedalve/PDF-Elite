...
export interface ToolOperationHook<TParams = unknown> {
  ...
  executeOperation: (params: TParams, files: File[]) => Promise<void>;
...
export function useToolOperation<TParams>(
  ...
  const storeResults = useCallback(async (resultFiles: File[]) => {
  ...
  const executeOperation = useCallback(
    async (params: TParams, inputFiles: File[]): Promise<void> => {
      if (isLoading) return;
      ...
      try {
        ...
        if (config.toolType === ToolType.custom && config.customProcessor) {
          ...
          const responseHandler = (config as MultiFileToolOperationConfig<TParams>).responseHandler;
          if (responseHandler) {
        ...
        await storeResults(resultFiles);
        setStatus("done");
      } catch (err: unknown) {
