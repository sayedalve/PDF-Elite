 ...
 */
export function useBaseTool<TParams extends { validateParameters: () => boolean }, TOperation extends {
  isLoading: boolean;
  files: File[];
  ...
  const handleExecute = useCallback(async () => {
    ...
    await operation.executeOperation(params, selectedFiles);
  }, [hasFiles, operation, params, selectedFiles]);
  ...
  const handleThumbnailClick = useCallback((_index: number) => {
