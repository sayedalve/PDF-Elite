...
export type OrganizeModeProps = {
  ...
  onApply?: (pages: { originalId: number; rotation: number }[]) => void;
  onExtract?: (selectedPageIds: number[]) => void;
  onInsert?: (selectedPageIds: number[]) => void;
  onReplace?: (selectedPageIds: number[]) => void;
  onSplit?: (selectedPageIds: number[]) => void;
  className?: string;
};

type PageItem = {
  id: string; // unique key for react
  originalId: number; // 1-indexed original page number
...
export const OrganizeMode: React.FC<OrganizeModeProps> = ({
      ...
      setInternalPages((prev) => {
        if (totalPages > prev.length) {
          const extra = Array.from(
            ...
            (_, i) => ({
              id: `new-${prev.length + i + 1}`,
              originalId: prev.length + i + 1,
              rotation: 0,
            }),
          );
          return [...prev, ...extra];
  ...
  const handleDelete = useCallback(() => {
    ...
    commitSelection([]);
  }, [effectiveSelected, selectedSet, commitSelection]);
        ...
        <div className="organize-toolbar">
          ...
          <div className="toolbar-group">
            <button
              className="toolbar-btn"
              onClick={() => {
                const pids = pages.filter(p => selectedSet.has(p.id)).map(p => p.originalId);
                onInsert?.(pids);
              }}
              title="Insert files"
            >
              ...
              return (
                ...
                >
                  ...
                  <div className="thumb-area">
                    ...
                    >
                      {/* Mock content - varies subtly by page */}
                  ...
                  <div className="card-footer">
                    <span className="page-number">Page {page.originalId}</span>
