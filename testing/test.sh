...
usage() {
    ...
    echo "  $0 --rerun \"pdf-elite-Regression pdf-elite-Security-Fat-with-login,Webpage-Accessibility-full\""
    exit 0
}
...
prepare_base_image() {
    if [ "${DOCKER_BASE_CHANGED:-false}" = "true" ]; then
        ...
        if docker build -f "$PROJECT_ROOT/docker/base/Dockerfile" \
            -t pdf-elite-base:local \
            "$PROJECT_ROOT/docker/base"; then
            echo "✓ Built base image locally: pdf-elite-base:local"
            BASE_IMAGE_ARG="--build-arg BASE_IMAGE=pdf-elite-base:local"
        else
...
capture_file_list() {
    ...
    docker exec "$container_name" sh -c "find / -type f \
        ...
        -not -path '/configs/*' \
        -not -path '/logs/*' \
        -not -path '*/home/PDFElitepdfuser/.config/libreoffice/*' \
        -not -path '*/home/PDFElitepdfuser/.config/calibre/*' \
        -not -path '*/home/PDFElitepdfuser/.java/fonts/*' \
        -not -path '*/home/PDFElitepdfuser/.pdfbox.cache' \
        -not -path '*/tmp/pdf-elite/PDFBox*' \
        -not -path '*/tmp/pdf-elite/hsperfdata_PDFElitepdfuser/*' \
        -not -path '*/tmp/hsperfdata_PDFElitepdfuser/*' \
        -not -path '*/tmp/hsperfdata_root/*' \
        -not -path '*/tmp/pdf-elite/jetty-*/*' \
        -not -path '*/tmp/pdf-elite/lu*' \
        -not -path '*/tmp/pdf-elite/tmp*' \
        -not -path '/tmp/lu*' \
        -not -path '*/tmp/*/user/registrymodifications.xcu' \
        -not -path '/app/PDFElite.aot' \
        -not -path '*/tmp/PDFElite.aotconf' \
...
main() {
    ...
    if should_run_test "pdf-elite-Ultra-Lite" || \
       should_run_test "Webpage-Accessibility-lite" || \
       should_run_test "pdf-elite-Ultra-Lite-Version-Check"; then
        ...
        if [ -n "${ACTIONS_RUNTIME_TOKEN}" ] && { [ -n "${ACTIONS_RESULTS_URL}" ] || [ -n "${ACTIONS_CACHE_URL}" ]; }; then
            DOCKER_CACHE_ARGS_ULTRA_LITE="--cache-from type=gha,scope=pdf-elite-ultra-lite --cache-to type=gha,mode=max,scope=pdf-elite-ultra-lite"
        else
        ...
        if ! docker buildx build --build-arg VERSION_TAG=alpha \
            -t docker.PDFElitepdf.com/PDFElitetools/pdf-elite:ultra-lite \
    ...
    run_tests "pdf-elite-Ultra-Lite" "./docker/embedded/compose/docker-compose-latest-ultra-lite.yml"
    ...
    if should_run_test "pdf-elite-Ultra-Lite-Version-Check"; then
        start_test_timer "pdf-elite-Ultra-Lite-Version-Check"
        echo "Testing version verification..."
        if verify_app_version "pdf-elite-Ultra-Lite" "http://localhost:8080"; then
            passed_tests+=("pdf-elite-Ultra-Lite-Version-Check")
            echo "Version verification passed for pdf-elite-Ultra-Lite"
        else
            failed_tests+=("pdf-elite-Ultra-Lite-Version-Check")
            capture_failure_logs "pdf-elite-Ultra-Lite-Version-Check" "$CURRENT_CONTAINER"
            echo "Version verification failed for pdf-elite-Ultra-Lite"
        fi
        stop_test_timer "pdf-elite-Ultra-Lite-Version-Check"
    fi
    ...
    if should_run_test "pdf-elite-Security-Fat" || \
       should_run_test "Webpage-Accessibility-full" || \
       should_run_test "pdf-elite-Security-Fat-Version-Check" || \
       should_run_test "pdf-elite-Security-Fat-with-login" || \
       should_run_test "pdf-elite-Regression pdf-elite-Security-Fat-with-login" || \
       should_run_test "pdf-elite-Fat-Disable-Endpoints" || \
       should_run_test "Disabled-Endpoints" || \
       should_run_test "pdf-elite-Fat-Disable-Endpoints-Version-Check"; then
        ...
        if [ -n "${ACTIONS_RUNTIME_TOKEN}" ] && { [ -n "${ACTIONS_RESULTS_URL}" ] || [ -n "${ACTIONS_CACHE_URL}" ]; }; then
            DOCKER_CACHE_ARGS_FAT="--cache-from type=gha,scope=pdf-elite-fat --cache-to type=gha,mode=max,scope=pdf-elite-fat"
        else
        ...
        if ! docker buildx build --build-arg VERSION_TAG=alpha \
            ${BASE_IMAGE_ARG} \
            -t docker.PDFElitepdf.com/PDFElitetools/pdf-elite:fat \
    ...
    run_tests "pdf-elite-Security-Fat" "./docker/embedded/compose/docker-compose-latest-fat-security.yml"
    ...
    if should_run_test "pdf-elite-Security-Fat-Version-Check"; then
        start_test_timer "pdf-elite-Security-Fat-Version-Check"
        echo "Testing version verification..."
        if verify_app_version "pdf-elite-Security-Fat" "http://localhost:8080"; then
            passed_tests+=("pdf-elite-Security-Fat-Version-Check")
            echo "Version verification passed for pdf-elite-Security-Fat"
        else
            failed_tests+=("pdf-elite-Security-Fat-Version-Check")
            capture_failure_logs "pdf-elite-Security-Fat-Version-Check" "$CURRENT_CONTAINER"
            echo "Version verification failed for pdf-elite-Security-Fat"
        fi
        stop_test_timer "pdf-elite-Security-Fat-Version-Check"
    fi
    ...
    # ==================================================================
    # PDFElite_PDF_TEST_COVERAGE=1 layers the JaCoCo agent override over
    ...
    if [ -n "${PDFElite_PDF_TEST_COVERAGE:-}" ]; then
        if [ ! -f "$PROJECT_ROOT/build/jacoco/jacocoagent.jar" ]; then
            echo "::warning::PDFElite_PDF_TEST_COVERAGE=1 but build/jacoco/jacocoagent.jar is missing - run ./gradlew copyJacocoAgent first"
        else
    ...
    run_tests "pdf-elite-Security-Fat-with-login" "./docker/embedded/compose/test_cicd.yml"

    # Only run behave tests if the container started successfully
    if [[ " ${passed_tests[*]} " =~ "pdf-elite-Security-Fat-with-login" ]]; then
        ...
        cd "testing/cucumber"
        start_test_timer "pdf-elite-Regression $CONTAINER_NAME"
        ...
        if python -m behave \
            ...
            if compare_file_lists "$BEFORE_FILE" "$AFTER_FILE" "$DIFF_FILE" "$CONTAINER_NAME"; then
                ...
                passed_tests+=("pdf-elite-Regression $CONTAINER_NAME")
            else
                ...
                test_failure_logs["pdf-elite-Regression-Temp-Files"]="$tempfile_log"
                failed_tests+=("pdf-elite-Regression-Temp-Files")
            fi
            passed_tests+=("pdf-elite-Regression $CONTAINER_NAME")
        else
            gha_endgroup
            failed_tests+=("pdf-elite-Regression $CONTAINER_NAME")
            ...
            test_failure_logs["pdf-elite-Regression $CONTAINER_NAME"]="$cucumber_log"
        ...
        stop_test_timer "pdf-elite-Regression $CONTAINER_NAME"
    fi
    ...
    # ==================================================================
    run_tests "pdf-elite-Fat-Disable-Endpoints" "./docker/embedded/compose/docker-compose-latest-fat-endpoints-disabled.yml"
    ...
    if should_run_test "pdf-elite-Fat-Disable-Endpoints-Version-Check"; then
        start_test_timer "pdf-elite-Fat-Disable-Endpoints-Version-Check"
        echo "Testing version verification..."
        if verify_app_version "pdf-elite-Fat-Disable-Endpoints" "http://localhost:8080"; then
            passed_tests+=("pdf-elite-Fat-Disable-Endpoints-Version-Check")
            echo "Version verification passed for pdf-elite-Fat-Disable-Endpoints"
        else
            failed_tests+=("pdf-elite-Fat-Disable-Endpoints-Version-Check")
            capture_failure_logs "pdf-elite-Fat-Disable-Endpoints-Version-Check" "$CURRENT_CONTAINER"
            echo "Version verification failed for pdf-elite-Fat-Disable-Endpoints"
        fi
        stop_test_timer "pdf-elite-Fat-Disable-Endpoints-Version-Check"
    fi
