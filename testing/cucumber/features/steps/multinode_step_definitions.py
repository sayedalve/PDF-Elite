...
def _psql(query):
    ...
    rc, out, err = _sh(
        ["docker", "exec", PG, "psql", "-U", "PDFElite", "-d", "PDFElite", "-tAc", query]
    )
...
def _network():
    ...
    return out.strip() or "compose_PDFElite-multinode"
...
def step_ledger_claim_atomic(context):
    ...
    rc, out, err = _sh(["docker", "exec", PG, "psql", "-U", "PDFElite", "-d", "PDFElite",
