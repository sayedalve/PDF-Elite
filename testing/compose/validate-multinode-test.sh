...
ADMIN_USER="admin"; ADMIN_PASS="PDFElite"
NODES="multinode-PDFElite-1 multinode-PDFElite-2"
...
a=$(count_of multinode-PDFElite-1); b=$(count_of multinode-PDFElite-2)
echo "  PDFElite-1 sources: $a   PDFElite-2 sources: $b"
if [ "$a" -gt 0 ] && [ "$a" = "$b" ]; then
...
users=$(docker exec multinode-postgres psql -U PDFElite -d PDFElite -tAc "select count(*) from users" 2>/dev/null | tr -d '[:space:]')
teams=$(docker exec multinode-postgres psql -U PDFElite -d PDFElite -tAc "select count(*) from teams" 2>/dev/null | tr -d '[:space:]')
conns=$(docker exec multinode-postgres psql -U PDFElite -d PDFElite -tAc "select count(*) from integration_configs" 2>/dev/null | tr -d '[:space:]')
...
echo " Stack left running: $LB  (admin / PDFElite)"
echo "============================================================"
[ "$fail" -eq 0 ]
