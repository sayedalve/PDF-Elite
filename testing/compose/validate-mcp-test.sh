...
if curl -sf http://localhost:8080/api/v1/info/status 2>/dev/null | grep -q "UP"; then
    pass "PDF Elite is UP"
else
    fail "PDF Elite is not UP"
fi
echo ""
...
# (no authorization_servers) makes clients fall back to treating PDFElite as its own AS.
...
USER_TOKEN=$(get_token "mcpuser@PDFElite.local" "mcppassword")
if [ -n "$USER_TOKEN" ]; then
    pass "Obtained access token for mcpuser@PDFElite.local"
else
    fail "Could not obtain access token for mcpuser@PDFElite.local (check Keycloak/client)"
fi
echo ""
...
if [ -n "$USER_TOKEN" ]; then
    ...
    for t in PDFElite_describe_operation PDFElite_convert PDFElite_pages PDFElite_misc PDFElite_security; do
...
# tools/call PDFElite_describe_operation
echo -e "${YELLOW}[5] Deep schema via PDFElite_describe_operation${NC}"
if [ -n "$USER_TOKEN" ]; then
    OP=$(echo "$LIST_BODY" | grep -oE '"enum":\[[^]]+\]' | head -1 | grep -oE '"[^"]+"' | sed -n '2p' | tr -d '"')
    if [ -n "$OP" ]; then
        echo -e "    ${BLUE}using operation:${NC} $OP"
        DESC_RPC="{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{\"name\":\"PDFElite_describe_operation\",\"arguments\":{\"operation\":\"$OP\"}}}"
...
# account-binding: valid Keycloak user with no PDFElite account -> 403
echo -e "${YELLOW}[6] Account-binding rejects users without a PDFElite account${NC}"
GHOST_TOKEN=$(get_token "ghost@PDFElite.local" "ghostpassword")
if [ -n "$GHOST_TOKEN" ]; then
    pass "Obtained a valid Keycloak token for ghost@PDFElite.local"
    ...
    if [ "$GHOST_CODE" = "403" ]; then
        pass "Valid token, no PDFElite account -> 403 (account-binding enforced)"
    else
        fail "ghost user -> $GHOST_CODE (expected 403)"
    fi
else
    fail "Could not obtain a token for ghost@PDFElite.local (cannot test account-binding)"
fi
echo ""
...
NONE_JWT="$(b64url '{"alg":"none","typ":"JWT"}').$(b64url '{"sub":"mcpuser","iss":"http://kubernetes.docker.internal:9080/realms/PDFElite-mcp","aud":"http://localhost:8080/mcp","email":"mcpuser@PDFElite.local","scope":"mcp.tools.read mcp.tools.write","exp":9999999999}')."
...
WRONG_AUD=$(curl -s -X POST "$TOKEN_URL" \
    ...
    --data-urlencode "username=mcpuser@PDFElite.local" \
...
if [ -n "$USER_TOKEN" ]; then
    ...
    CAT=$(curl -s -X POST "$MCP_URL" -H "Content-Type: application/json" \
        -H "Authorization: Bearer $USER_TOKEN" \
        -d '{"jsonrpc":"2.0","id":81,"method":"tools/call","params":{"name":"PDFElite_security","arguments":{"operation":"add-password"}}}')
...
if [ -z "$USER_TOKEN" ]; then
    ...
    EXEC_RPC=$(printf '{"jsonrpc":"2.0","id":91,"method":"tools/call","params":{"name":"PDFElite_pages","arguments":{"operation":"rotate-pdf","fileName":"example.pdf","parameters":{"angle":90},"file":"%s"}}}' "$B64")
