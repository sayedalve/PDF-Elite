...
wait_up() {
    local w=0
    while [ $w -lt 360 ]; do
        if [ "$(docker inspect -f '{{.State.Health.Status}}' pdf-elite-mcp-test 2>/dev/null)" = "healthy" ]; then
            echo ""
            return 0
        fi
...
echo -e "${YELLOW}▶ Recreating PDFElite in apikey mode...${NC}"
MCP_AUTH_MODE=apikey PREMIUM_KEY="${PREMIUM_KEY:-}" \
    docker compose -f "$COMPOSE" up -d --no-build --force-recreate pdf-elite-mcp >/dev/null 2>&1
if ! wait_up; then
    fail "PDFElite did not become healthy in apikey mode"
...
echo -e "${YELLOW}▶ Minting a PDFElite API key for mcpuser...${NC}"
...
if [ -n "$APIKEY" ]; then
    ...
    if [ "$OK_CODE" = "200" ] && grep -q "PDFElite_describe_operation" /tmp/mcp_apikey_list.json; then
...
PREMIUM_KEY="${PREMIUM_KEY:-}" docker compose -f "$COMPOSE" up -d --no-build --force-recreate pdf-elite-mcp >/dev/null 2>&1
wait_up && pass "restored oauth mode" || fail "PDFElite not healthy after restoring oauth"
echo ""
