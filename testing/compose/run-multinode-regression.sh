...
if ! docker inspect -f '{{.State.Health.Status}}' multinode-PDFElite-1 2>/dev/null | grep -q healthy; then
...
if [ "$RUN_FAILOVER" = 1 ]; then
  ...
  for n in multinode-PDFElite-1 multinode-PDFElite-2; do
...
echo " Stack left running: http://localhost:8080  (admin / PDFElite)"
echo "============================================================"
exit $rc
