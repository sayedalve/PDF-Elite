...
echo -e "${BLUE}╔════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  PDF Elite + Keycloak SAML Test Environment   ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════╝${NC}"
echo ""
...
while [ $WAITED -lt $MAX_WAIT ]; do
    if curl -sf http://localhost:9080/realms/PDFElite-saml/protocol/saml/descriptor 2>/dev/null | grep -q "EntityDescriptor"; then
...
if [ ! -f "$PRIVATE_KEY" ] || [ ! -f "$PUBLIC_CERT" ]; then
    openssl req -x509 -newkey rsa:2048 -keyout "$PRIVATE_KEY" -out "$PUBLIC_CERT" \
        -days 3650 -nodes -subj "/CN=pdf-elite-saml-sp" >/dev/null 2>&1
...
CERT_BODY="$(curl -sf http://localhost:9080/realms/PDFElite-saml/protocol/saml/descriptor \
...
echo ""
echo -e "${YELLOW}▶ Starting PDF Elite...${NC}"
docker-compose -f docker-compose-keycloak-saml.yml up "${COMPOSE_UP_ARGS[@]}" pdf-elite-saml
