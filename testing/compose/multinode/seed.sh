...
ADMIN_PASS="${ADMIN_PASS:-PDFElite}"
USER_COUNT="${USER_COUNT:-40}"
USER_PASS="${USER_PASS:-Password123!}"
PGHOST="${PGHOST:-postgres}"
PGUSER="${PGUSER:-PDFElite}"
PGPASSWORD="${PGPASSWORD:-PDFElite}"
PGDATABASE="${PGDATABASE:-PDFElite}"
export PGPASSWORD
...
while [ "$n" -le "$USER_COUNT" ]; do
  uname=$(printf "user%02d@PDFElite.test" "$n")
...
log "  users: user01..$(printf '%02d' "$USER_COUNT")@PDFElite.test / $USER_PASS"
