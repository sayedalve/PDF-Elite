...
print_manual_panel() {
    ...
    echo -e "     ${YELLOW}mcpuser@PDFElite.local${NC} / ${YELLOW}mcppassword${NC}"
    echo ""
    echo -e "${BLUE}Stop:${NC} docker-compose -f docker-compose-keycloak-mcp.yml down -v"
    echo ""
}

# Manual API-key panel: mint a PDFElite per-user key and print header-based client settings.
print_manual_panel_apikey() {
    local jwt key
    jwt=$(curl -s -X POST http://localhost:8080/api/v1/auth/login \
        -H "Content-Type: application/json" \
        -d '{"username":"mcpuser@PDFElite.local","password":"mcppassword"}' \
    ...
    echo ""
    echo -e "   ${BLUE}No browser redirect, no IdP. The key maps to ${YELLOW}mcpuser@PDFElite.local${BLUE} and${NC}"
...
echo -e "${BLUE}╔════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   PDF Elite + Keycloak MCP Test Environment   ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════╝${NC}"
echo ""
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo "  --apikey              Manual testing mode (API key): bring PDFElite up in apikey"
...
if [ "${SKIP_MCP_PREFLIGHT:-false}" != "true" ]; then
    if ! curl -sf --connect-timeout 2 --max-time 3 "http://${KEYCLOAK_HOST}:9080/realms/PDFElite-mcp" >/dev/null 2>&1; then
...
while [ $WAITED -lt $MAX_WAIT ]; do
    if curl -sf http://localhost:9080/realms/PDFElite-mcp > /dev/null 2>&1; then
...
echo ""
echo -e "${YELLOW}▶ Starting PDF Elite (MCP resource server)...${NC}"
docker-compose -f docker-compose-keycloak-mcp.yml up "${COMPOSE_UP_ARGS[@]}" pdf-elite-mcp

echo ""
echo -e "${YELLOW}▶ Waiting for PDF Elite...${NC}"
WAITED=0
while [ $WAITED -lt $MAX_WAIT ]; do
    if curl -sf http://localhost:8080/api/v1/info/status 2>/dev/null | grep -q "UP"; then
        echo -e "${GREEN}✓ PDF Elite is ready${NC}"
        break
    fi
    echo -n "."
    sleep 2
...
if [ $WAITED -ge $MAX_WAIT ]; then
    echo -e "${RED}✗ PDF Elite failed to start${NC}"
    exit 1
fi
...
echo -e "   PDF Elite:   ${GREEN}http://localhost:8080${NC}"
...
echo ""
echo -e "${BLUE}👤 Test user (exists in Keycloak AND as a PDFElite account):${NC}"
echo -e "     Email:    ${GREEN}mcpuser@PDFElite.local${NC}"
echo -e "     Password: ${GREEN}mcppassword${NC}"
echo ""
if [ "$APIKEY_MODE" != true ]; then
    echo -e "${BLUE}👻 Negative-test user (valid Keycloak login, NO PDFElite account):${NC}"
    echo -e "     Email:    ${GREEN}ghost@PDFElite.local${NC}"
