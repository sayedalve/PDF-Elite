...
echo "==> Building the PDFElite image (first run compiles the app; be patient)..."
$COMPOSE build
...
for node in multinode-PDFElite-1 multinode-PDFElite-2; do
...
============================================================================
 Multi-node PDFElite is UP.

   App (via load balancer): http://localhost:8080     (admin / PDFElite)
   MinIO console:           http://localhost:9001     (minioadmin / minioadmin)
   Postgres:                localhost:5434            (PDFElite / PDFElite, db 'PDFElite')

   Seeded users:            user01..user40@PDFElite.test / Password123!
   Global API key:          multinode-test-key   (header: X-API-KEY)

 Try it:
   ./validate-multinode-test.sh        # multi-node smoke tests (optional)
   $COMPOSE logs -f PDFElite-1         # tail a node
   ./start-multinode-test.sh --down    # stop + wipe

 Nodes are reachable directly for cross-node checks:
   docker compose -f docker-compose-multinode.yml exec PDFElite-1 curl -s localhost:8080/api/v1/info/status
============================================================================
EOF
